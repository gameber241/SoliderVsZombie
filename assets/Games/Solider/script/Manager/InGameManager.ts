...
export class InGameManager extends BaseSingleton<InGameManager> {
    ...
    Show() {
        ...
        tween(opacity).to(1, { opacity: 255 })
            .start()
    ...
    PLayGame() {
        this.scheduleOnce(() => {
            InGameManager.getInstance().SetIdSolider(LobbyManager.getInstance().soliderCurrent == LobbyManager.getInstance().solider_0 ? ESOLIDER.SOLIDER_0 : ESOLIDER.SOLIDER_1)
